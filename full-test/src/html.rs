...
pub fn render(ctx: &ReportContext<'_>) -> String {
    ...
    render_benchmarks(&mut out, ctx.bench);
...
fn render_phase_summary(out: &mut String, ctx: &ReportContext<'_>) {
    ...
    // Bench (WARN-only locally; release-thresholded issues are FAIL-level)
    let b = ctx.bench;
    let b_release_issues = b.release_blocking_issues();
    ...
    let b_failed_cls = if !b_release_issues.is_empty() {
        " fail"
    } else if b.build_failed || b_crashed > 0 {
        " warn"
    } else {
        ""
    };
    let b_failed_disp = if !b_release_issues.is_empty() {
        format!("{} release issue(s)", b_release_issues.len())
    } else if b.build_failed {
        "build-failed".to_string()
    } else {
        b_crashed.to_string()
    };
...
fn render_benchmarks(out: &mut String, bench: &BenchResult) {
    out.push_str("<h2>Benchmark table</h2>\n");
    out.push_str(&format!(
        "<p class=\"meta\">Profile: <code>{}</code> &bull; Claim: {}</p>\n",
        html_escape(bench.profile.as_str()),
        html_escape(bench.profile.claim_note()),
    ));
    if let Some(source) = bench.profile.threshold_source() {
        out.push_str(&format!(
            "<p class=\"meta\">Threshold source: {}</p>\n",
            html_escape(source),
        ));
    }
    let release_issues = bench.release_blocking_issues();
    ...
    out.push_str("</table>\n");
    render_benchmark_thresholds(out, bench);
}

fn render_benchmark_thresholds(out: &mut String, bench: &BenchResult) {
    let rows = bench.threshold_rows();
    if rows.is_empty() {
        return;
    }
    out.push_str("<h2>Benchmark release thresholds</h2>\n");
    out.push_str("<table>\n<tr><th>Vector</th><th>Fixture</th><th class=\"num\">Enc baseline</th><th class=\"num\">Enc fail</th><th>Enc status</th><th class=\"num\">Dec baseline</th><th class=\"num\">Dec fail</th><th>Dec status</th><th>Issue</th></tr>\n");
    for row in rows {
        let enc_cls = threshold_status_class(row.enc_status);
        let dec_cls = threshold_status_class(row.dec_status);
        out.push_str(&format!(
            "<tr><td>{}</td><td><code>{}</code></td><td class=\"num\">{}</td><td class=\"num\">{}</td><td class=\"{}\">{}</td><td class=\"num\">{}</td><td class=\"num\">{}</td><td class=\"{}\">{}</td><td>{}</td></tr>\n",
            html_escape(&row.label),
            html_escape(row.relative_path.as_deref().unwrap_or("n/a")),
            html_escape(&cell_ratio(row.enc_baseline_ratio)),
            html_escape(&cell_ratio(row.enc_release_fail_ratio)),
            enc_cls,
            html_escape(row.enc_status.as_str()),
            html_escape(&cell_ratio(row.dec_baseline_ratio)),
            html_escape(&cell_ratio(row.dec_release_fail_ratio)),
            dec_cls,
            html_escape(row.dec_status.as_str()),
            html_escape(row.issue.as_deref().unwrap_or("")),
        ));
    }
    out.push_str("</table>\n");
}

fn threshold_status_class(status: crate::bench::BenchThresholdStatus) -> &'static str {
    match status {
        crate::bench::BenchThresholdStatus::Pass => "pass",
        crate::bench::BenchThresholdStatus::Fail
        | crate::bench::BenchThresholdStatus::MissingRequiredFixture
        | crate::bench::BenchThresholdStatus::Crashed
        | crate::bench::BenchThresholdStatus::MissingTiming
        | crate::bench::BenchThresholdStatus::MissingThreshold => "fail",
    }
}
...
mod tests {
    ...
    #[test]
    fn release_thresholded_benchmarks_render_claim_and_threshold_rows() {
        let opts = Options {
            release_preflight: true,
            ..Options::default()
        };
        ...
        assert!(html.contains("class=\"pass\">pass"));
    }
