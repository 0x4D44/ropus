...
| `sample-count-only` | The test checks shape/length only; sample values are not a correctness oracle. |
...
| Fuzz | Shared fuzz oracle weakened classes | `sample-count-only` | Target-specific corpus/crash input | Fuzzing exercises active targets and applies weakened sample-count-only oracle classes where exact parity is deliberately disabled. | Strong PCM parity for paths deliberately downgraded to sample-count-only. | `debt` | `none` | `/home/md/language/ropus/wrk_docs/2026.05.06 - HLD - fuzz target discovery.md` | 2026-05-06 |
| Fuzz | Shared fuzz oracle exact classes | `exact-pcm-parity` | Target-specific corpus/crash input | Fuzzing applies exact PCM parity for generated cases that remain in the strong oracle class. | Coverage of weakened oracle classes. | `warn` | `none` | `/home/md/language/ropus/wrk_docs/2026.05.06 - HLD - fuzz target discovery.md` | 2026-05-06 |
