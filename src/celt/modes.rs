...
pub fn init_caps(m: &CELTMode, cap: &mut [i32], lm: i32, c: i32) {
    for i in 0..m.nb_ebands as usize {
        ...
        cap[i] = ((m.cache.caps[(m.nb_ebands * (2 * lm + c - 1)) as usize + i] as i32) + 64)
            * c * n >> 2;
    }
}
