#!/usr/bin/env bash
#
# Campaign 6 monitor — check status, crashes, and stats for all running fuzzers.
#

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CAMPAIGN_DIR="$ROOT/logs/fuzz-findings/campaign6"

echo "=== Campaign 6 Status @ $(date +%H:%M:%S) ==="
echo ""
...
if [[ -d "$ROOT/fuzz_crashes" ]]; then
    ...
    if [[ $panic_crashes -gt 0 ]]; then
        echo "  fuzz_crashes/ (panic capture): $panic_crashes files"
