...
fn main() {
    ...
    while off + 8 <= buf.len() {
        ...
        let fin_range = u32::from_be_bytes([buf[off+4], buf[off+5], buf[off+6], buf[off+7]]);
        off += 8;
        let lost = (len_word & 0x80000000) != 0;
        let len = (len_word & 0x7FFFFFFF) as usize;
        if lost { lost_count += 1; }
        if frame < 50 || lost {
            println!("frame {}: {}bytes{}, final_range=0x{:08x}", frame, len, if lost {" [LOST]"} else {""}, fin_range);
        }
        if off + len > buf.len() {
