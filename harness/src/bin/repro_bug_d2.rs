//! Reproducer for Campaign 8 Bug D (second attempt), using the saved
//! byte-exact fuzz crash file.
//!
//! Input: `fuzz_crashes/crash_047eb2739268a41f.bin`
//! Expected: frame 1 diverges between Rust and C encoders.
//!
//! Usage: cargo run --release --bin repro_bug_d2
...
fn main() {
    let path = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "fuzz_crashes/crash_047eb2739268a41f.bin".to_string());
    ...
    // Simple summary of first few samples of first two frames
    for fi in 0..num_frames.min(3) {
        let f = &pcm_frames[fi];
    ...
    match first_mismatch {
        ...
        Some(i) => {
            ...
            let cp = &c_packets[i];
            println!("  rust[0..{}] = {:?}", rp.len(), rp);
            println!("  c   [0..{}] = {:?}", cp.len(), cp);
        }
    }
}
